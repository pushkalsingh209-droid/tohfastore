...
16. **Split `app/admin/page.tsx`** — *in progress (2026-08-29).* Plan:
    ...
    `AdminDataContext`) + `SecurityTab` + `ReviewsTab` + `CouponsTab` (page −287 lines so
    far, −11 `no-explicit-any`). **Left:** `overview`, `orders`, `settings`, `products`
    (the big one) — one PR each.
