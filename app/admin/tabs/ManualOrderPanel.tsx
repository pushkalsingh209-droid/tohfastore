...
export default function ManualOrderPanel() {
  ...
  return (
    <div className="bg-white border border-stone-200 rounded-lg p-4 sm:p-6 mb-6 shadow-sm">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
      ...
      >
        <span>
          <span className="block text-sm font-bold text-stone-900">Record a manual order</span>
          <span className="block text-[11px] text-stone-500 mt-0.5">
            For a sale closed on WhatsApp or a payment link — keeps stock, the sold count and the GST report correct.
          </span>
        </span>
        <span className="text-stone-400 text-xs shrink-0">{open ? "Hide" : "Open"}</span>
      </button>
        ...
        <div className="mt-4 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <select
              ...
              className="flex-1 min-w-0 px-3 py-2 rounded border border-stone-300 text-sm bg-stone-50 focus:outline-none focus:border-amber-600"
            >
            ...
            <input
              ...
              className="w-full sm:w-20 px-3 py-2 rounded border border-stone-300 text-sm font-mono text-right bg-stone-50 focus:outline-none focus:border-amber-600"
            />
            <button
              type="button"
              onClick={addLine}
              disabled={!pickId}
              className="min-h-[40px] px-4 rounded bg-stone-900 text-white text-xs uppercase font-semibold tracking-wider disabled:bg-stone-200 disabled:text-stone-400"
            >
              Add
            </button>
          </div>
          ...
          {lines.length > 0 && (
            <div className="border border-stone-200 rounded divide-y divide-stone-100">
                ...
                <div key={l.id} className="flex items-center justify-between gap-2 px-3 py-2 text-sm">
                  ...
                  <span className="flex items-center gap-3 shrink-0">
                    ...
                    <button
                      ...
                      className="text-rose-600 text-xs uppercase font-semibold"
                    >
                      Remove
                    </button>
                  </span>
              ...
              <div className="flex items-center justify-between px-3 py-2 text-sm font-bold bg-stone-50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <input
              ...
              className="px-3 py-2 rounded border border-stone-300 text-sm bg-stone-50 focus:outline-none focus:border-amber-600"
            />
            <input
              value={phone}
              ...
              className="px-3 py-2 rounded border border-stone-300 text-sm font-mono bg-stone-50 focus:outline-none focus:border-amber-600"
            />
            <input
              value={email}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input
              ...
              className="flex-1 min-w-[200px] px-3 py-2 rounded border border-stone-300 text-sm font-mono bg-stone-50 focus:outline-none focus:border-amber-600"
            />
          </div>
          ...
          {result && <p className="text-xs text-emerald-800 bg-emerald-50 border border-emerald-200 rounded p-2">{result}</p>}

          <button
            type="button"
            onClick={submit}
            disabled={saving || lines.length === 0}
            className="w-full sm:w-auto min-h-[44px] px-6 rounded bg-amber-700 hover:bg-amber-800 text-white text-xs uppercase font-semibold tracking-wider disabled:bg-stone-200 disabled:text-stone-400"
          >
