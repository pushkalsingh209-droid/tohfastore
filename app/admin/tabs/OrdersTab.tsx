  ...
  return (
    ...
    <div className="bg-surface border border-border rounded-lg shadow-sm p-6 mb-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-4">
        <div>
          <h2 className="text-lg font-serif text-fg">Notifications sent</h2>
        ...
        <div className="flex items-end gap-2">
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-faint font-semibold mb-1">From</label>
            <input
              type="date"
              value={notifFrom}
              onChange={(e) => setNotifFrom(e.target.value)}
              max={notifTo || undefined}
              className="px-2.5 py-1.5 rounded border border-border-strong text-xs focus:outline-none focus:border-accent bg-surface-2"
            />
          </div>
          <div>
            <label className="block text-[10px] uppercase tracking-wider text-faint font-semibold mb-1">To</label>
            <input
              type="date"
              value={notifTo}
          ...
          {(notifFrom || notifTo) && (
            <button
              ...
              className="px-2.5 py-1.5 rounded border border-border-strong text-xs font-semibold text-faint hover:bg-surface-2 transition"
            >
              Reset
            </button>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {ORDER_STATUS_TABS.filter((t) => t.key !== "all").map((t) => (
          <div key={t.key} className="bg-surface-2 border border-border rounded-lg p-3">
            <p className="text-[10px] uppercase tracking-wider text-faint font-semibold mb-1">{t.label}</p>
            <p className="text-lg font-mono font-bold text-fg">{notifTotals.byStatus[t.key] ?? 0}</p>
          </div>
        ))}
        <div className="bg-accent-soft border border-accent-soft-border rounded-lg p-3">
          <p className="text-[10px] uppercase tracking-wider text-accent font-semibold mb-1">Total</p>
          <p className="text-lg font-mono font-bold text-accent">{notifTotals.total}</p>
        </div>
      </div>
    </div>
    ...
    <div className="bg-surface border border-border rounded-lg shadow-sm p-8">
      <div className="border-b border-border pb-4 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-serif text-fg">Settled Customer Transactions</h2>
            <p className="text-faint text-xs mt-1">Real-time purchase streams verified and pushed directly by your Razorpay webhook endpoint script.</p>
          </div>
      ...
      {loadingOrders ? (
        <p className="text-faint text-sm text-center py-6 animate-pulse">Syncing transactions ledger from cloud data cache...</p>
      ) : orders.length === 0 ? (
        <p className="text-faint text-sm text-center py-6">No payment captured records generated yet.</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left font-sans text-xs sm:text-sm text-muted border-collapse">
            <thead>
              <tr className="bg-surface-2 text-muted uppercase font-semibold text-[11px] tracking-wider border-b border-border">
                <th className="p-4">#</th>
              ...
              {paginatedOrders.map((order, index) => (
                <tr key={order.id} className="hover:bg-surface-2/50 transition">
                  ...
                  <td className="p-4 font-mono font-bold text-fg">
                    {order.payment_id}
                    <span className="block text-[10px] text-faint font-normal mt-1">
                      Order ID: {order.order_id}
                    </span>
                    <span className="block text-[10px] text-faint font-normal mt-0.5">
                      {new Date(order.created_at).toLocaleString("en-IN")}
                    </span>
                  </td>
                  <td className="p-4 font-light">
                    <div className="font-normal text-fg">{order.customer_details?.name || "Pushkal Singh"}</div>
                    <div className="text-xs text-faint mt-0.5">{order.customer_details?.email}</div>
                    <div className="text-xs text-faint mt-0.5">{order.customer_details?.contact}</div>
                    {order.shipping_address && (
                      <div className="text-[11px] text-faint mt-1.5 max-w-[240px] leading-snug space-y-0.5 border-t border-border pt-1.5">
                        <div>
                          <span className="uppercase tracking-wide text-faint font-semibold">Ship to: </span>
                          {order.shipping_address.line}
                        </div>
                        {order.shipping_address.landmark && (
                          <div className="text-faint">Near {order.shipping_address.landmark}</div>
                        )}
                        <div>
                        ...
                        {order.shipping_address.recipientPhone && (
                          <div className="mt-1 inline-block px-1.5 py-0.5 rounded bg-accent-soft text-accent font-semibold">
                    ...
                    <div className="space-y-1">
                      {Array.isArray(order.items) ? (
                        order.items.map((item, idx) => (
                          <div key={idx} className="text-xs text-muted font-light">
                            • <span className="font-normal text-fg">{item.name}</span>
                            <span className="text-accent font-medium ml-1">x{item.quantity}</span>
                          </div>
                        ))
                      ) : (
                        <span className="text-faint text-xs">Standard checkout package</span>
                      )}
                    </div>
                  </td>
                  <td className="p-4">
                      ...
                      return (
                        <div className="mt-2 space-y-1.5">
                          <input
                            ...
                            className="block w-44 px-2 py-1.5 rounded border border-border text-[11px] font-mono focus:outline-none focus:border-accent bg-surface-2 placeholder:text-faint"
                          />
                          <select
                            value={selectValue}
                            ...
                            className="block w-44 px-2 py-1.5 rounded border border-border text-[11px] focus:outline-none focus:border-accent bg-surface-2 text-muted"
                          >
                          ...
                          {showOther && (
                            <input
                              ...
                              className="block w-44 px-2 py-1.5 rounded border border-border text-[11px] focus:outline-none focus:border-accent bg-surface-2 placeholder:text-faint"
                            />
                          )}
                        </div>
                      );
                    ...
                    <button
                      ...
                      className="mt-2 inline-flex items-center gap-1.5 rounded border border-accent-soft-border bg-accent-soft px-2.5 py-1.5 text-[11px] font-semibold text-accent hover:bg-accent-soft transition"
                    >
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-x-2 gap-y-0.5 text-[9px] font-mono text-faint">
                      {ORDER_STATUS_TABS.filter((t) => t.key !== "all").map((t) => (
                        <span key={t.key} className={countFor(order.id, t.key) > 0 ? "text-faint font-semibold" : undefined}>
                          {t.label} ({countFor(order.id, t.key)})
                        </span>
                      ))}
                    </div>
                  </td>
                  <td className="p-4 text-right font-mono font-bold text-accent text-base">
                    ...
                    {order.payment_method === "cod" ? (
                      <span className="mt-1 block">
                        <span className="inline-block rounded border border-accent-soft-border bg-accent-soft px-2 py-0.5 text-[10px] font-sans font-bold uppercase tracking-wide text-accent">
                          COD &mdash; collect on delivery
                        </span>
                        {order.cod_fee != null && (
                          <span className="mt-0.5 block text-[10px] font-sans font-normal text-faint">
                        ...
                        {order.cod_collected_at ? (
                          <button
                            ...
                            className="mt-1 inline-block rounded border border-success-border bg-success-soft px-2 py-0.5 text-[10px] font-sans font-bold uppercase tracking-wide text-success"
                          >
                          ...
                          <button
                            ...
                            className="mt-1 inline-block rounded border border-border-strong px-2 py-0.5 text-[10px] font-sans font-semibold uppercase tracking-wide text-muted hover:bg-surface-2"
                          >
                            Mark cash collected
                          </button>
                        )}
                      ...
                      <span className="mt-1 block text-[10px] font-sans font-normal text-faint">Paid online</span>
                    )}
                  </td>
                </tr>
              ))}
    ...
    {notifyOrder && (
      <div
        className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-fg/40 sm:p-4"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div
          className="w-full sm:max-w-lg bg-surface rounded-t-2xl sm:rounded-xl shadow-xl max-h-[92vh] overflow-y-auto"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-start justify-between gap-3 p-4 sm:p-5 border-b border-border">
            <div>
              <h3 className="text-base font-serif font-bold text-fg">Notify customer</h3>
              <p className="text-[11px] text-faint mt-0.5 font-mono">{notifyOrder.order_id}</p>
            </div>
            <span
          ...
          <div className="p-4 sm:p-5 space-y-4">
            <p className="text-xs text-faint leading-relaxed">
              Sends a <strong className="text-muted">{notifyStatus}</strong> update to{" "}
            ...
            <div>
              ...
              />
              <p className="text-[10px] text-faint mt-0.5 text-right">
              ...
              {notifyExtraNumbers.trim() && (
                <p className="text-[10px] mt-1 space-y-0.5">
                  {notifyExtraParsed.valid.length > 0 && (
                    <span className="block text-success">Will send to: {notifyExtraParsed.valid.join(", ")}</span>
                  )}
                  {notifyExtraParsed.invalid.length > 0 && (
                    <span className="block text-danger">Not a valid number, skipped: {notifyExtraParsed.invalid.join(", ")}</span>
                  )}
                  {notifyExtraParsed.truncated && (
                    <span className="block text-accent">Only the first {MAX_EXTRA_NOTIFY_NUMBERS} are sent to.</span>
                  )}
                </p>
              )}
            </div>
            ...
            <div>
              <p className="text-[11px] uppercase tracking-wider font-semibold text-faint mb-1">Preview</p>
              <pre className="whitespace-pre-wrap break-words text-xs text-muted bg-surface-2 border border-border rounded p-3 font-sans">
                {notifyPreview}
              </pre>
            </div>

            {notifyResult && (
              <div className="text-xs rounded border border-border bg-surface-2 p-3 space-y-1">
                <p className={notifyResult.whatsapp === "sent" ? "text-success" : notifyResult.whatsapp === "failed" ? "text-danger" : "text-faint"}>
                  {channelText(notifyResult.whatsapp, "WhatsApp", "not configured or no number on file")}
                </p>
                <p className={notifyResult.email === "sent" ? "text-success" : notifyResult.email === "failed" ? "text-danger" : "text-faint"}>
                  {channelText(notifyResult.email, "Email", "no email on file")}
                </p>
                {notifyResult.extra.map((r) => (
                  <p key={r.number} className={r.result === "sent" ? "text-success" : r.result === "failed" ? "text-danger" : "text-faint"}>
                    {channelText(r.result, `WhatsApp to ${r.number}`, "WhatsApp not configured")}
                  </p>
                ))}
                {notifyResult.extraInvalid.length > 0 && (
                  <p className="text-danger">Skipped, not a valid number: {notifyResult.extraInvalid.join(", ")}</p>
                )}
          ...
          <div className="flex gap-2 p-4 sm:p-5 border-t border-border sticky bottom-0 bg-surface">
            <button
              ...
              className="flex-1 py-2.5 rounded border border-border-strong text-sm font-semibold text-muted hover:bg-surface-2 transition disabled:opacity-50"
            >
            ...
            <button
              ...
              onClick={sendNotify}
              disabled={notifySending}
              className="flex-1 py-2.5 rounded bg-accent text-accent-fg text-sm font-semibold hover:bg-accent transition disabled:opacity-50"
            >
