  ...
  return (
    <div className="bg-bg min-h-screen flex flex-col justify-between transition-colors">
      ...
      <div className="flex-grow py-12 md:py-16 px-4 sm:px-6 font-sans text-muted">
        <div className="max-w-3xl mx-auto bg-surface border border-border rounded-lg p-6 sm:p-8 md:p-12 shadow-sm">
          <div className="mb-6">
            <h1 className="text-2xl sm:text-3xl font-serif text-fg mb-2 font-medium tracking-wide">
              Frequently Asked Questions
            </h1>
            <p className="text-faint text-xs sm:text-sm font-light">
              Shipping, payments, returns, and more -- still have a question? WhatsApp or email us any time from our{" "}
              <a href="/contact" className="text-link underline">
                Contact page
              </a>
              .
            </p>
            ...
            {FAQS.map((item, index) => {
              ...
              return (
                <div key={item.question} className="py-1">
                  ...
                  >
                    <span className="text-sm sm:text-base font-serif font-medium text-fg">
                      {item.question}
                    </span>
                    <span
                      className={`flex-shrink-0 text-link text-lg leading-none transition-transform ${isOpen ? "rotate-45" : ""}`}
                      aria-hidden="true"
                    >
                      +
                    </span>
                  ...
                  {isOpen && (
                    <p className="text-xs sm:text-sm text-muted font-light leading-relaxed pb-4 pr-8">
                      {item.answer}
                    </p>
                  )}
                </div>
