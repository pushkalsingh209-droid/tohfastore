  ...
  if (checkingOut && cart.length > 0) {
    return (
      <div className="fixed inset-0 z-50 overflow-hidden">
        <div className="absolute inset-0 bg-scrim/40 backdrop-blur-sm" onClick={closeDrawer} />
      ...
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-surface shadow-xl flex flex-col h-full">
          ...
          <div className="p-6 border-b border-border flex items-center justify-between">
            ...
            <button onClick={closeDrawer} className="text-faint hover:text-muted text-sm font-medium">✕ Close</button>
          </div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                <div className="space-y-4 max-h-[35vh] overflow-y-auto border-b pb-4">
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-center gap-4 pb-2">
                      <div className="relative w-12 h-12 rounded overflow-hidden border bg-surface-2 flex-shrink-0">
                      ...
                      <button onClick={() => removeFromCart(item.id)} className="text-faint hover:text-danger text-[11px] self-start">Remove</button>
                    </div>
                  ))}
                </div>
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-1.5 py-2 text-[11px] font-semibold uppercase tracking-wider text-success border border-success-border rounded hover:bg-success-soft transition"
                >
